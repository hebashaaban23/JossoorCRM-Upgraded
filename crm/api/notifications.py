...
def mark_portal_seen(name: str, source: str = "Notification Log"):
    ...
    if source == "Notification Log":
        ...
        # Use found column, or fallback to 'read' or 'seen' based on attr
        target_col = seen_col
        if not target_col:
            if hasattr(doc, "read"): target_col = "read"
            elif hasattr(doc, "seen"): target_col = "seen"
            
        if target_col:
            setattr(doc, target_col, 1)
